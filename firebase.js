...
const init=()=>{
  ...
  // Safety timeout — if nothing responds in 10s, fall through to login
  const authTimeout = setTimeout(() => {
    document.getElementById('globalLoader').classList.add('hidden');
    showScreen('loginScreen');
  }, 10000);

  // On mobile, after Google redirect the page reloads.
  // getRedirectResult() resolves with the user — handle it explicitly
  // before onAuthStateChanged, which may not fire reliably on GitHub Pages cross-domain.
  checkRedirectResult().then(result => {
    if (result && result.user) {
      // Got a user from redirect — process immediately, cancel the auth listener race
      clearTimeout(authTimeout);
      handleAuthStateChange(result.user);
    } else {
      // No redirect result — fall through to normal auth state listener
    ...
    // Redirect check failed — still listen for auth state
    onAuthStateChanged(auth, (user) => {
      clearTimeout(authTimeout);
      handleAuthStateChange(user);
    });
